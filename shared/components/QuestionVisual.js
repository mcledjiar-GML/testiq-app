...
/**
 * 🎨 COMPOSANT D'AFFICHAGE DES VISUELS PRÉ-STOCKÉS
 * ===============================================
 * 
 * Affiche les visuels pré-générés et stockés en BD
 * Plus de génération dynamique - garantit la cohérence
 */
  ...
  useEffect(() => {
    const loadVisual = async () => {
      try {
        ...
        console.log(`🎨 Chargement du visuel pré-stocké pour ${questionId}...`);
        
        // **NOUVEAU** : Récupération depuis BD au lieu de génération dynamique
        const response = await api.get(`/api/questions/${questionId}/visual`);

        if (response.data.success && response.data.hasVisual) {
          setVisual(response.data.visualData);
          setHasVisual(true);
          console.log(`✅ Visuel pré-stocké chargé pour ${questionId}`);
        } else {
          setHasVisual(false);
          console.log(`ℹ️ Pas de visuel stocké pour ${questionId}`);
        }
